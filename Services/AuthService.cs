    ...
    namespace Diploma.Services
    {
        ...
        public async Task<bool> SendResetCodeAsync(string email)
        {
            ...
            var user = await connection.QueryFirstOrDefaultAsync<User>(
                "SELECT UserId, Username, LastResetRequest FROM Users WHERE Email = @Email",
                new { Email = email });

            if (user == null)
                throw new Exception("Пользователь с таким email не найден");

            // Проверяем, не отправляли ли код слишком часто (например, в течение последних 2 минут)
            if (user.LastResetRequest.HasValue &&
                (DateTime.Now - user.LastResetRequest.Value).TotalMinutes < 2)
            {
                double minutesLeft = 2 - (DateTime.Now - user.LastResetRequest.Value).TotalMinutes;
                throw new Exception($"Повторный запрос можно отправить через {Math.Ceiling(minutesLeft)} минут(ы)");
            }

            // Генерируем новый код
            string resetCode = new Random().Next(100000, 999999).ToString();

            // Обновляем коды и время последнего запроса
            await connection.ExecuteAsync(
                @"UPDATE Users 
          SET ResetCode = @ResetCode, 
              ResetCodeExpiry = DATE_ADD(NOW(), INTERVAL 15 MINUTE),
